...
export function image2d(pixels, x, y, image, sx, sy, sw, sh) {
...
export function sprite2d(pixels, x, y, scale, image, sx, sy, sw, sh) {
  const iw = image.width
  const src = image.pixels
  let s = y * CANVAS_WIDTH
  let f = sy * iw
...
function texspan2d(pixels, image, x1, u1, v1, x2, u2, v2, line) {
  ...
  const utexel = 1.0 / wide
  const vtexel = 1.0 / image.height
...
export function centrictexture2d(pixels, image, x0, y0, u0, v0, x1, y1, u1, v1, x2, y2, u2, v2) {
  ...
  const max_x = Math.min(Math.max(Math.max(x0, x1), x2), CANVAS_WIDTH)
  const max_y = Math.min(Math.max(Math.max(y0, y1), y2), CANVAS_HEIGHT)
...
export function mode2d(pixels, image, x1, y1, x2, y2, x3, y3, x4, y4, sl, st, sr, sb) {
  centrictexture2d(pixels, image, x1, y1, sl, st, x2, y2, sr, st, x3, y3, sl, sb)
  centrictexture2d(pixels, image, x3, y3, sl, sb, x2, y2, sr, st, x4, y4, sr, sb)
}
